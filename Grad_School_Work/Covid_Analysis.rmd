...
# percent population infected  ###################### GOOD ONE ########################3
plotly_build(ggplot(hdi_check, aes(x=date, y=percent_pop_infected, color = location)) +
  geom_line() +
  my_theme +
...
ggplot(filter(hdi_check, location=='United States') ,aes(x=date, y=stringency_index, color = location)) +
  geom_line() +
  my_theme +
  scale_x_date(breaks = "30 days") 
par(new= TRUE)
ggplot(filter(hdi_check, location=='United Kingdom') ,aes(x=date, y=stringency_index, color = location)) +
  geom_line(hdi_check, aes(x=date, y=deaths)) +
  my_theme +
  scale_x_date(breaks = "30 days")


ggplot(filter(hdi_check, location == 'United States'), aes(x=date, y=pct_change)) +
  geom_point(na.rm=TRUE) +
  my_theme

ggplot(filter(hdi_check, location == 'United States'), aes(x=date, y=stringency_index)) +
  geom_smooth(na.rm=TRUE) +
  my_theme

ggplot(filter(hdi_check, location == 'United States'), aes(x=date, y=new_cases)) +
  geom_smooth(na.rm=TRUE) +
  my_theme

ggplot(filter(hdi_check, location == 'United States'), aes(x=date, y=cases_21_days)) +
  geom_point(na.rm = TRUE) +
  my_theme
...
## diabetes
plotly_build(ggplot(total_deaths, aes(x=location, y =diabetes_prevalence))
             + geom_bar(stat="identity") + my_theme 
             #+ scale_y_continuous(trans="log2")
             ) 
...
male_smokers = ggplot(total_deaths, aes(x=location, y=male_smokers)) +
  geom_bar(stat = "identity") + my_theme
...
smokers_all_genders = ggplot(smokers, aes(x = location, y = value, fill = location)) + 
  geom_bar(stat = "identity") + my_theme

deaths = ggplot(total_deaths, aes(x = location, y = total_deaths)) + 
  geom_bar(stat = "identity") + my_theme +
  scale_y_continuous(trans = "log2")

multiplot(cardio_death, smokers_all_genders, diabetes,  cols=2)
